...
function get_infos(){
    ...
    #}'
    cut -f 1,2,6,12 $1 | perl -ane  'if(/NM:i:(\d+)/){ $n=$1;$m=$g=$o=0;$m+=$1 while/(\d+)M/g;$g+=$1,++$o while/(\d+)[ID]/g; chomp ;@a=split;print($a[0],"\t",$a[1],"\t",1-($n-$g+$o)/($m+$o) ,"\t",($m+$o)-($n-$g+$o),  "\t",($m+$o),"\n")}'
}
...
function get_scores(){
    ...
    awk 'BEGIN{name="";score=0;}{ if(NR>1){ n=$1; if(n!=name && name!=""){printf("%s\t%f\n",name,score);score=0;} name=n;if($2<256){score+=(3*(log($3)/log(10))+(log($5)/log(10)) );} } }' $1
}
